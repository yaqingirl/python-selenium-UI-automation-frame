...
from config.server_info import api_bookdetail, api_recommandbooks, api_get_twenty_comments
import requests
...
def getData(ebookid):
    ...
    url=api_get_twenty_comments%(ebookid)
    ...
    return result
def getCommentsByPopular(ebookid):
    ...
    # print(response.request.headers)
    # book_list=re.findall(r'"ebook_id":(\d+)',response.text)
    result=json.loads(response.text)['data']['ebook_comment_infos']

    return result
def getCommentsByCreattime(ebookid):
...
if __name__=="__main__":
    ...
    print(getCommentsByCreattime('30331055'))
