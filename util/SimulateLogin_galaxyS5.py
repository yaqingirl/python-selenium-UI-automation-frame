...
def get_position(browser):
    ...
    imgrab.save(r'../img/im.png',"PNG")
    im = Image.open(r'../img/im.png')
    im = im.crop((46, 335, 766, 1615))
    im.save(r'../img/im.png')
    ...
    im_crop = im.crop((508,846,548,886))
    ...
    # 下面是步骤1、2
    # 我的im_grey.png原始大小是720*1280，计算后需要缩小1.3倍
    res = cv2.resize(cv2.imread(r'../img/im_grey.png'), (554, 985), interpolation=cv2.INTER_CUBIC)  # 该方法可缩放图片
    ...
    # 下面是步骤3：
    # 以上计算的坐标是在缩小1.3倍以后的图片上，想要计算在原始图中的坐标需要把坐标*1.3
    real_pos = (int(pos['result'][0] * 1.3), int(pos['result'][1] * 1.3))
    #print(real_pos)
...
def verify_code(browser):
    ...
    while flag:
        ...
        #im.png大小是720*1280，浏览器最大化的大小是360*640，刚好截图大小是网页中html大小的2倍，所以坐标需要除以2
        ##但是由于浏览器最大化，im.png的分辨率太大，导致定位稍微不准确就不能定位，需要优化，失败后可多次重试定位
        x=int(real_pos[0]/2)
        y=int(real_pos[1]/2)
        #如果点击范围不在弹出的验证码弹框范围内，则进行刷新，重新定位
        if not (x>42 and x<317 and y>228 and y<399) :
            time.sleep(1)
            logger.info('x=%s和y=%s不在范围内容'%(str(x),str(y)))
            browser.find_element_by_xpath('// *[ @ class = "jcap_refresh"] ').click()
            # refresh_button=getElement(browser,'xpath','// *[ @ id = "captcha"] / div[1] / div')
...
if __name__=="__main__":
    ...
    browser.find_element_by_xpath('//input[@id="password"]').send_keys("你的密码")
